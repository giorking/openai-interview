...
def run_test(input_csv, output_csv, model="gpt-3.5-turbo", temperature=0.0, system_content="You are a helpful assistant.", user_content_header="Please find the airline names in this tweet: "):
    ...
    def find_airline_names(tweet, index):
        ...
        parsed_completion = chat_completion.replace("'", "").replace("[", "").replace("]", "").replace('"', "")
        # airline_names will always be an array of strings
        airline_names = parsed_completion.split(',')
        airline_names = [name.strip() for name in airline_names]
